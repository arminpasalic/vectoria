...
async function initialize() {
  ...
  const ret = {
    ...
    memory_bytes: () => module.HEAPU8.buffer.byteLength,
...
function allocateMemory(ctx, bytes, purpose) {
  const ptr = ctx.memory_allocate(bytes);
  if (bytes > 0 && ptr === 0) {
    throw new Error(
      `UMAP WASM allocation failed (${purpose}, ${bytes} bytes).` +
      " Try reducing the dataset size or rebuild the runtime with a larger memory budget."
    );
  }
  return ptr;
}
...
function set_options(ctx, ptr, setters, options) {
  ...
  const buffer = allocateMemory(ctx, buffer_length * 2, "option buffers");
...
export async function createUMAP(count, input_dim, output_dim, data, options = {}) {
  ...
  const bytes_input = count * input_dim * 4;
  const bytes_output = count * output_dim * 4;
  const ptr_input = allocateMemory(ctx, bytes_input, "UMAP input");
  const ptr_output = allocateMemory(ctx, bytes_output, "UMAP output");
...
export async function createKNN(count, input_dim, data, options) {
  ...
  const ptr_input = allocateMemory(ctx, count * input_dim * 4, "KNN input");
  ctx.f32_array(ptr_input, count * input_dim).set(data, 0);
  ...
  let ptr_buffer = allocateMemory(ctx, 10 * 4, "KNN scratch buffer");
  const ptr_q = allocateMemory(ctx, input_dim * 4, "KNN query buffer");

  function get_buffers(k) {
    let required_capacity = k * 2;
    if (buffer_capacity < required_capacity) {
      ctx.memory_free(ptr_buffer);
      ptr_buffer = allocateMemory(ctx, required_capacity * 4 * 2, "KNN scratch buffer resize");
